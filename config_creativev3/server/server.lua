...
checkPermission = function(user_id,raceID) -- Função que checa a permissão para iniciar a corrida.
    return not (vRP.hasPermission(user_id,police) or vRP.hasPermission(user_id,"Paramedic") ) -- Não permite que policiais, paramédicos, e pessoas "procuradas" iniciem a corrida.
end
